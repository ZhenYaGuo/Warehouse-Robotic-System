...
def main():
    ...
    while not rospy.is_shutdown():
    	#Read ar tag translation in camera frame
        #raw_input('Press [ Enter ]: ')

        """
        translation = [int(404),int(404),int(404)]
        translation= GetARtagDistance(tfBuffer,'left_hand_camera','ar_marker_8')
        if translation == [int(404),int(404),int(404)]:
            print("AR tag not found")
            break
        
        if abs(translation[2]-desired_position_camera_artag[2])<0.04 and abs(translation[0]-desired_position_camera_artag[0])<0.04 and abs(translation[1]-desired_position_camera_artag[1])<0.04:
            print("Get desired position")
            reach = 1

        #print(translation)
        base_camera_translation = [int(404),int(404),int(404)]
        base_camera_translation= GetARtagDistance(tfBuffer,'base','left_hand_camera')
        if base_camera_translation == [int(404),int(404),int(404)]:
            print("AR tag not found")
            break
        
        error_camera_artag = [desired_position_camera_artag[0]-translation[0] , desired_position_camera_artag[1]-translation[1] , desired_position_camera_artag[2]-translation[2]]
        translation_error_accu = [translation_error_accu[0]+error_camera_artag[0] , translation_error_accu[1]+error_camera_artag[1] , translation_error_accu[2]+error_camera_artag[2]]

        #print("Artag_camera translation",translation)
        print("camera_base translation",base_camera_translation)
        
        if not reach:
            target_translation = PID(error_camera_artag,base_camera_translation,translation_error_accu)
            #print("target_translation",target_translation)

        #if not reach:
            #Baxter_Go(target_translation,compute_ik)
        """
        rospy.sleep(1)
